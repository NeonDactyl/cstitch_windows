...
void helpBrowser::processPrintAll() {
  ...
  // concatenate them
  QRegularExpression head("<html>.*</head>");
  QRegularExpression next("Next: *<a.*\\n");
  //next.setMinimal(true);
  QRegularExpression top("Top: *<a.*\\n");
  //top.setMinimal(true);
  QRegularExpression previous("Previous: *<a.*\\n");
  //previous.setMinimal(true);
  QRegularExpression foot("</body>.*");
  QRegularExpression newUser("<h1>New User.*</p>");
  //newUser.setMinimal(true);
  QRegularExpression dmc("<h1>A Note On DMC.*</p>");
  //dmc.setMinimal(true);
