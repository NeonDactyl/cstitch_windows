...
patternMetadata::patternMetadata(int pdfWidth, int titleFontSize,
  ...
  const int dWidth = (*this->screen()).virtualSize().width();
  const int dHeight = (*this->screen()).virtualSize().height();
  scroll->setMinimumWidth(qMin(dWidth - 100, inputFieldWidth + 100));
  scroll->setMinimumHeight(qMin(dHeight - 100, 600));
