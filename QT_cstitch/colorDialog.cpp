...
void colorDialog::fitDialog() {
  ...
  /*
   * TODO:
   * const QScreen thisScreen = new QScreen();
   * convert below QApplication desktop dimensions to use QScreen objects
   * 
   */

  const int desktopWidth = (*this->screen()).virtualSize().width();
  const int desktopHeight = (*this->screen()).virtualSize().height();
