...
void imageSaverWindow::doPdfSave(const QString& outputFile,
  ...
  const int xMargins =
    printer.paperRect(QPrinter::Inch).width() - printer.pageRect(QPrinter::Inch).width();
  const int yMargins =
    printer.paperRect(QPrinter::Inch).height() - printer.pageRect(QPrinter::Inch).height();
  QPageSize pageSize(QSizeF(image.width() + xMargins,
      image.height() + yMargins), QPageSize::Inch);
