...
QVector<pairOfInts> xmlToCoordinatesList(const QString& list) {
  ...
  for (int i = 0, size = coordinates.size(); i < size; ++i) {
    QRegularExpressionMatch match = regex.match(coordinates[i]);
    if (match.hasMatch()) {
...
QVector<pixel> xmlToPixelList(const QString& list) {
  ...
  for (int i = 0, size = pixels.size(); i < size; ++i) {
    QRegularExpressionMatch match = regex.match(pixels[i]);
    if (match.hasMatch()) {
      QStringList matches = match.capturedTexts();
      if (matches.size() == 6) {
        QList<int> matchInts;
...
QVector<historyPixel> xmlToHistoryPixelList(const QString& list) {
  ...
  QRegularExpression regex("^\\{\\[\\((\\d+),(\\d+)\\)\\.\\((\\d+),(\\d+),(\\d+)\\)\\]:\\((\\d+),(\\d+),(\\d+)\\):(true|false)\\}$");
  for (int i = 0, size = stringList.size(); i < size; ++i) {
    QRegularExpressionMatch match = regex.match(stringList[i]);
    if (match.hasMatch()) {
      QStringList matches = match.capturedTexts();
      if (matches.size() == 10) {
        QList<int> matchInts;
